...
from utilities.bytbit_perp import PerpBybit
...
async def main():
    ...
    try:
        ...
        try:
            print(f"Setting {margin_mode} for account...")
            await exchange.set_margin_mode(margin_mode)
        except Exception as e:
            print(e)
        try:
            print(f"Setting x{exchange_leverage} on {len(pairs)} pairs...")
            tasks = [
                exchange.set_leverage(pair, exchange_leverage)
                for pair in pairs
            ]
        ...
        await exchange.cancel_all_triggers()
        ...
        tasks = [exchange.get_open_positions(pair) for pair in pairs]
