...
class PerpBybit:
    ...
    async def get_last_ohlcv(self, pair, timeframe, limit=1000) -> pd.DataFrame:
        ...
        while current_ts < end_ts:
            req_end_ts = min(current_ts + (bybit_limit * ts_dict[timeframe]), end_ts)
            tasks.append(
            ...
            current_ts += (bybit_limit * ts_dict[timeframe]) + 1
    ...
    async def get_balance(self) -> UsdtBalance:
        resp = await self._session.fetch_balance({"type":"fund","accountType":"UNIFIED"})
        return UsdtBalance(
    ...
    async def set_margin_mode(self, margin_mode):
        ...
        try:
            await self._session.set_margin_mode(margin_mode)
        except Exception as e:
            print('Error:', e)
            pass
        return Info(
            success=True,
            message=f"Margin mode set to {margin_mode}",
        )
    ...
    async def get_open_positions(self, pair) -> List[Position]:
        pair = self.ext_pair_to_pair(pair)
        ...
        for position in resp:
            if position['entryPrice'] != None and position['entryPrice'] != 0:
                liquidation_price = 0
                take_profit_price = 0
                stop_loss_price = 0
                unrealized_pnl = 0
    ...
    async def get_open_orders(self, pair) -> List[Order]:
        pair = self.ext_pair_to_pair(pair)
        resp = await self._session.fetch_open_orders(pair, params={"orderFilter": "Order"})
        return_orders = []
        for order in resp:
    ...
    async def cancel_all_orders(self):
        try:
            resp = await self._session.cancel_all_orders(params={"orderFilter": "Order"})
            return Info(success=True, message=f"{len(resp)} All orders cancelled")
        except Exception as e:
            print('Error:', e)
    ...
    async def cancel_all_triggers(self):
        try:
            resp = await self._session.cancel_all_orders(params={"orderFilter": "StopOrder"})
            return Info(success=True, message=f"{len(resp)} All trigger Orders cancelled")
        except Exception as e:
            print('Error:', e)
    ...
    async def cancel_trigger_orders(self, pair, ids=[]):
        try:
            ...
            print('Error:', e)
            return Info(success=False, message="Error or no triggers to cancel")
